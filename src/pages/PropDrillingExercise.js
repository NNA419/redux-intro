...
const RootComponent = (props) => {
  ...
  const addProductToCart = (obj) => { 
    ...
    let total = cart.totalPrice + obj.price;
    setCart({ products: newProduct, totalPrice: total })
    console.log(cart)
  };
  ...
  const removeProductFromCart = (obj) => {
    ...
    const removeProduct = cart.products.map((product) => {
      if (product.id === obj.id && product.qty > 0) {
        product.price -= obj.price;
        product.qty--;
        total -= obj.price;
      }
      ...
      setCart({ products: removeProduct, totalPrice: total });
      console.log(cart);
    console.log("khò khò")
  }
  // Step 3
  ...
  return (
    <WrapperBox>
      ...
      <Grid container spacing={2} p="1rem">
        <Grid item md={6}>
          <ProductPage
            products={products} click={addProductToCart}
      ...
      <Grid container spacing={2} p="1rem">
        <Grid item sm={6}>
          <ProductOne
            ...
            remove={props.remove}
          />
          {/* <h1>{props.products[1].price}</h1> */}
        </Grid>
        <Grid item sm={6}>
          <ProductTwo
            product1={props.products[1]}
            click={props.click}
            remove={props.remove}
          />
        </Grid>
...
const ProductOne = (props) => {
   console.log(props);
  //props = { id: "p1", title: "Product 1", price: 1999 },
  return (
    <WrapperBox>
      <Typography
        p="0.5rem"
        ...
        <Grid item xs={8}>
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            <Button
              ...
              size="sm"
              style={{ width: "5rem" }}
              onClick={() => props.click(props.product1)}
            >
              Add
            </Button>
            <Button
              variant="error" size="sm" style={{ width: "5rem" }}
              onClick={() => props.remove(props.product1)}
            >
              Remove
            </Button>
          </div>
